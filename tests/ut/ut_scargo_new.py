...
def test_create_toml_file(tmpdir):
    os.chdir(tmpdir)
    scargo_new("test_project", None, None, TARGET_X86, False, False)
    assert os.path.exists("scargo.toml")


def test_create_src_dir(tmpdir):
    os.chdir(tmpdir)
    scargo_new("test_project", None, None, TARGET_X86, False, False)
    assert os.path.exists("src")


def test_create_test_dir(tmpdir):
    os.chdir(tmpdir)
    scargo_new("test_project", None, None, TARGET_X86, False, False)
    assert os.path.exists("tests")


def test_src_content(tmpdir):
    os.chdir(tmpdir)
...
def test_test_content_dir(tmpdir):
...
def test_with_git_dir_exist(tmpdir):
    os.chdir(tmpdir)
...
def test_without_git_dir_exist(tmpdir):
    os.chdir(tmpdir)
    scargo_new("test_project", None, None, TARGET_X86, False, False)
    project_dir_list = [dir for dir in os.listdir()]
