...
def test_fix_pragma(create_new_project: None, caplog: pytest.LogCaptureFixture, get_lock_file: Config) -> None:
    lock_file = get_lock_file
    check_pragma(lock_file, True)
    assert "Fixed" in caplog.text


def test_check_copyright(create_new_project: None, caplog: pytest.LogCaptureFixture, get_lock_file: Config) -> None:
    lock_file = get_lock_file
